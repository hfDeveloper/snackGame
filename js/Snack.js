...
//二、定义计分面板类
class ScorePanel {
    constructor(maxLevel = 10, addLevelStep = 5) {
...
class Snack {
    constructor() {
        this.isHorizontal = true; //此属性用于判断蛇是否在水平方向移动
        this.isVertical = false; //此属性用于判断蛇是否在垂直方向移动
    ...
    moveAllBody() {
        for (let i = this.bodyEles.length - 1; i > 0; i--) {
            //获取前面一节身体的位置
            let x = this.bodyEles[i - 1].offsetLeft; //TS类型断言，告诉TS这就是HTML元素
            let y = this.bodyEles[i - 1].offsetTop;
            //设置
            this.bodyEles[i].style.left = x + "px";
            this.bodyEles[i].style.top = y + "px";
        }
    }
    //检查蛇头是否撞到自己身体的方法
    CrashSelf() {
        for (let i = 1; i < this.bodyEles.length; i++) {
            let bodyEle = this.bodyEles[i];
...
class GameControl {
    constructor() {
        this.direction = 'Right'; //用于存储蛇运动方向(也就是按键代表的方向，按键按下时给其赋值;默认游戏开始向右移动)
        this.isAlive = true; //创建一个属性用于判断游戏是否结束
